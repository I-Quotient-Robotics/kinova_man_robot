...
class JointTrajectoryAction(object):
    ...
    def __init__(self, name):
        ...
        self._driver_name = rospy.get_param("~driver_name", "j2s7s300_driver")
        self._controller_name = rospy.get_param("~controller_name", "j2s7s300")

        self._as = actionlib.SimpleActionServer('/'+self._controller_name+'/follow_joint_trajectory', control_msgs.msg.FollowJointTrajectoryAction, execute_cb=self.execute_cb, auto_start = False)
        self._feedback_sub = rospy.Subscriber('/'+self._driver_name+'/trajectory_controller/state', control_msgs.msg.FollowJointTrajectoryFeedback, self.feedback_cb)
        self._controller = pid_controller.PIDController()
        self._vel_pub = rospy.Publisher('/'+self._driver_name+'/in/joint_velocity',
    ...
    def execute_cb(self, goal):
        ...
        self._controller.load_trajectory(goal.trajectory, self._driver_name)
